...
local function parse_types_file(path)
  ...
  for _, raw in ipairs(lines) do
    ...
    if str.startswith(line, "---") then
      ...
      local assign_target = line:match("^([%w%._%[%]\"']+)%s*=%s*function%s*%(([^)]*)%)")
      local assign_params
      if assign_target then
        assign_target, assign_params = line:match("^([%w%._%[%]\"']+)%s*=%s*function%s*%(([^)]*)%)")
      end
      ...
      if fn_full then
        name = fn_full:match("[^%.:]+$")
        params = params or ""
      elseif assign_target then
        local key = assign_target:match("^M%.([%w_]+)$")
          or assign_target:match('^M%["([^"]+)"%]$')
          or assign_target:match("^M%['([^']+)'%]$")
        if key then
          name = key
          params = assign_params or ""
        end
      end
